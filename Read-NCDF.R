...
do <- list(c(as.Date("2013-06-17"), as.Date("2013-12-31")),
           c(as.Date("2014-01-01"), as.Date("2014-06-24")))
...
time = NULL
time = lapply(ncl, ncvar_get, varid='time')
time = lapply(time, function(t) (t - 48) * 3600) # convert to seconds for R
time = lapply(time, as.POSIXct, origin='1-1-1 00:00:00', tz='GMT')
time = lapply(time, as.Date)

# set the Z slot with the date extracted from layer names
bl <- lapply(1:length(bl), function(i){
  setZ(bl[[i]], time[[i]], name = 'date')
} )
...
ra13 = read.csv(file.path(grive, file_names[1]))
ra14 = read.csv(file.path(grive, file_names[2]))
ra13$date = as.Date(ra13$date)
ra14$date = as.Date(ra14$date)

date_seq = seq(do[[1]][1], do[[2]][2], by='day')
df = data.frame('date'=date_seq)

df = merge(df, rbind(ra13, ra14), by='date')
write.csv(df, file.path(grive, 'Reanalysis-Data.csv'))